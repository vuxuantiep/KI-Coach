...
GROQ_API_KEY = "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"
...
def call_groq(prompt: str, system_prompt: str = "", max_tokens: int = 1000) -> str:
    """Generiert Antwort über Groq Cloud API."""
...
{cv_excerpt}"""
    ...
    raw = call_groq(prompt, system_prompt=system, max_tokens=600)
...
def optimize_cv(cv_text: str, user_name: str, target_role: str = "", analysis: dict | None = None) -> str:
    ...
    return call_groq(prompt, system_prompt=system, max_tokens=1800)
...
with st.sidebar:
    ...
    st.info(f"🚀 Groq Modell: {GROQ_MODEL}")
    ...
    st.divider()
    st.caption("Smart Coach Pro v2.0 | Powered by Groq Cloud + D-ID")
...
with tab_chat:
    ...
    if send_btn and user_input and user_input.strip():
        ...
        with st.spinner("Coach antwortet..."):
            ...
            response = call_groq(full_prompt, system_prompt=system)
        ...
        if st.session_state.optimized_cv:
            ...
            if st.button("💬 Im Chat besprechen", use_container_width=True):
                ...
                with st.spinner("Coach antwortet..."):
                    response = call_groq(
                        msg,
                        system_prompt=build_coach_system(user_name, st.session_state.optimized_cv)
                    )
                st.session_state.chat_history.append({"role": "assistant", "content": response})
...
with tab_ikigai:
    ...
    if step < len(IKIGAI_FRAGEN):
        ...
        with st.spinner("🌸 Erstelle deine IKIGAI-Analyse..."):
            ...
            result = call_groq(prompt, system_prompt=system, max_tokens=1000)
